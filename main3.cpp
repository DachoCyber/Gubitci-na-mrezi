...
void napraviKonekcije(std::vector<Konekcija>& skupKonekcija, const std::vector<Vod>& vodovi, int brStubova, int trVodInd, const std::vector<Potrosac>& potrosaci) {
    ...
    for (int i = trVodInd; i < vodovi.size(); i++) {
        for (int j = vodovi[i].stubovi.size() - 1; j > 0; j--) {
            ...
            if (!postoji) {
                ...
                std::cout << p1.geogDuzina << " " << p2.geogDuzina << std::endl;
                std::cout << (p1.geogDuzina - p2.geogDuzina) * 109000 * cos(p2.geogSirina * 3.14159 / 180) << std::endl;
                ...
                if (vodovi[i].stubovi[j].id != vodovi[i].stubovi[j - 1].id) {
                    Konekcija knkcija = {vodovi[i].stubovi[j], vodovi[i].stubovi[j - 1], 0, rastojanje, otpor};
...
int main() {
    ...
    napraviKonekcije(skupKonekcija, vodovi, brStubova, 0, potrosaci);
